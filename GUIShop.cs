 ...
 * Add Theme Switcher
 * Seperate All Button config option into 2 toggle config options.. 
 * Add BLuePrint Generation option
 * Add TC or Backpack option to store custom currency (seeking alternitive)..
 *******************************************************/
 ...
 *
 * This update 2.2.44
 * Adds cooldown option/setting to prevent players from being able to shop after x time has passed each wipe cycle.
*/

namespace Oxide.Plugins
{
    [Info("GUIShop", "Khan", "2.2.43")]
    ...
    public class GUIShop : RustPlugin
    {
        ...
        //Caching Shop Images.
        private const string WelcomeImage = "Welcome";
        private const string BackgroundImage = "Background";
        private const string Amount1Image = "Amount1";
        private const string Amount2Image = "Amount2";
        private const string BuyImage = "Buy";
        private const string SellImage = "Sell";
        private const string BackArrowImage = "BackArrow";
        private const string ForwardArrowImage = "ForwardArrow";
        private const string CloseImage = "Close";
        private const string ShopButton = "Button";
        ...
        {
            /*[JsonProperty("Don't Edit This")]
            public DontEdit Time = new DontEdit();*/
            ...
            [JsonProperty("Player UI display")] public bool PersonalUI = false;

            [JsonProperty("Block Monuments")] public bool BlockMonuments = false;

            [JsonProperty("If true = Images, If False = Text Labels")]
            public bool UIImageOption = false;

            [JsonProperty("NPC Distance Check")] public float NPCDistanceCheck = 2.5f;

            [JsonProperty("Enable NPC Auto Open")] public bool NPCAutoOpen = false;
            ...
            [JsonProperty("Shop - Sell $ Label")] public string SellLabel = "Sell $";
            ...
            [JsonProperty("Shop - Close Label")] public string CloseButtonlabel = "CLOSE";
        ...
        {
            public readonly Dictionary<string, int> BuyPrice = new Dictionary<string, int>();
            public readonly Dictionary<string, int> SellPrice = new Dictionary<string, int>();

            public bool HasSellLimit(string item, int amount)
            {
                if (!SellPrice.ContainsKey(item))
                {
                    SellPrice[item] = 1;
                }
        ...
        /*public class DontEdit
        {
            public bool WasWiped;
            public string WipeTime;
            public string LastWipe;
            [JsonProperty("Sets the cooldown before being able to use the shop after server wipes")]
        ...
        private void LoadImages()
        {
            ...
            foreach (ShopItem shopItem in _config.ShopItems.Values)
            {
                ...
                if (!shopItem.Image.IsNullOrEmpty() && !imageListGUIShop.ContainsKey(shopItem.Image))
                {
                    imageListGUIShop.Add(shopItem.Image, shopItem.Image);
                }

                itemIcons.Add(new KeyValuePair<string, ulong>(shopItem.Shortname, shopItem.SkinId));
            }

            imageListGUIShop.Add(BackgroundImage, _config.BackgroundUrl);
            imageListGUIShop.Add(WelcomeImage, _config.GuiShopWelcomeUrl);
            imageListGUIShop.Add(Amount1Image, _config.AmountUrl);
            imageListGUIShop.Add(Amount2Image, _config.AmountUrl2);
            imageListGUIShop.Add(BuyImage, _config.BuyIconUrl);
            imageListGUIShop.Add(SellImage, _config.SellIconUrl);
            imageListGUIShop.Add(BackArrowImage, _config.BackButtonUrl);
            imageListGUIShop.Add(ForwardArrowImage, _config.ForwardButtonUrl);
            imageListGUIShop.Add(CloseImage, _config.CloseButton);
            imageListGUIShop.Add(ShopButton, _config.GUI.Image);
            imageListGUIShop.Add(_config.IconUrl, _config.IconUrl);
            //ImageLibrary?.Call("AddImage", _config.BackgroundUrl, BackgroundImage);
            //Puts($"{_config.BackgroundUrl} {_config.GUI.Image}");
            //ImageLibrary?.Call("AddImage", _config.GuiShopWelcomeUrl, WelcomeImage);
            // ImageLibrary?.Call("AddImage", _config.AmountUrl, Amount1Image);
            // ImageLibrary?.Call("AddImage", _config.AmountUrl2, Amount2Image);
            // ImageLibrary?.Call("AddImage", _config.BuyIconUrl, BuyImage);
            // ImageLibrary?.Call("AddImage", _config.SellIconUrl, SellImage);
            // ImageLibrary?.Call("AddImage", _config.BackButtonUrl, BackArrowImage);
            // ImageLibrary?.Call("AddImage", _config.ForwardButtonUrl, ForwardArrowImage);
            // ImageLibrary?.Call("AddImage", _config.CloseButton, CloseImage);
            // ImageLibrary?.Call("AddImage", _config.GUI.Image, ShopButton);
            ...
            ImageLibrary?.Call("ImportImageList", Title, imageListGUIShop, 0UL, true, new Action(ShopReady));
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop did not recieve a response from the Economics plugin. Please ensure the Economics plugin is installed correctly. "
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop did not recieve a response from the ServerRewards plugin. Please ensure the ServerRewards plugin is installed correctly. "
                },
                {"MessageErrorTakeCurrency", "GUIShop has failed to take this currency upon purchase {0}"},
                ...
                {"MessageErrorNotKit", "This is not a valid kit name assigned to this shop item {0}"},
                ...
                {"MessageErrorCustomCurrency", "WANRING Admin has this shop called {0} set as custom currency but has not set a valid currency ID {1}"},
                ...
                {"MessageErrorItemIsNotBlueprintable", "This shop item {0} cannot be set as a Blueprint!"},
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop n'a pas reçu de réponse du plugin Economics. Veuillez vous assurer que le plugin Economics est correctement installé."
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop n'a pas reçu de réponse du plugin ServerRewards. Veuillez vous assurer que le plugin ServerRewards est correctement installé."
                },
                ...
                {
                    "MessageErrorItemItem",
                    "AVERTISSEMENT: il semble que cet article que vous possédez n'est pas un article valide! Veuillez contacter un administrateur!"
                },
                {
                    "MessageErrorItemNoValidbuy",
                    "AVERTISSEMENT: Il semble que ce ne soit pas un article valide à acheter, veuillez contacter un administrateur!"
                },
                {
                    "MessageErrorItemNoValidsell",
                    "AVERTISSEMENT: Il semble que ce ne soit pas un article valide à vendre, veuillez contacter un administrateur!"
                },
                {
                    "MessageErrorRedeemKit",
                    "AVERTISSEMENT: Une erreur s'est produite lors de la remise de ce kit, veuillez contacter un administrateur!"
                },
                {"MessageErrorBuyCmd", "Impossible d'acheter plusieurs exemplaires de cet article!"},
                {
                    "MessageErrorBuyPrice",
                    "AVERTISSEMENT: aucun prix d'achat n'a été donné par l'administrateur, vous ne pouvez pas acheter cet article"
                },
                {
                    "MessageErrorSellPrice",
                    "AVERTISSEMENT: aucun prix de vente n'a été donné par l'administrateur, vous ne pouvez pas vendre cet article"
                },
                ...
                {
                    "MessageErrorItemNoExist",
                    "AVERTISSEMENT: l'article que vous essayez d'acheter ne semble pas exister! Veuillez contacter un administrateur!"
                },
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop fick inte svar från pluginet Economics. Se till att plugin för ekonomi är korrekt installerad."
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop fick inte svar från ServerRewards-plugin. Se till att ServerRewards-tillägget är korrekt installerat."
                },
                ...
                {
                    "MessageErrorItemItem",
                    "VARNING: Det verkar som att detta säljföremål du har inte är ett giltigt objekt! Vänligen kontakta en administratör!"
                },
                {
                    "MessageErrorItemNoValidbuy",
                    "VARNING: Det verkar som om det inte är ett giltigt objekt att köpa. Kontakta en administratör!"
                },
                {
                    "MessageErrorItemNoValidsell",
                    "VARNING: Det verkar som om det inte är ett giltigt objekt att sälja. Kontakta en administratör!"
                },
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop heeft geen reactie ontvangen van de Economics-plug-in. Zorg ervoor dat de Economics-plug-in correct is geïnstalleerd."
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop heeft geen antwoord ontvangen van de ServerRewards-plug-in. Zorg ervoor dat de ServerRewards-plug-in correct is geïnstalleerd."
                },
                ...
                {
                    "MessageErrorItemItem",
                    "WAARSCHUWING: het lijkt erop dat dit verkoopartikel dat u heeft geen geldig item is! Neem contact op met een beheerder!"
                },
                {
                    "MessageErrorItemNoValidbuy",
                    "WAARSCHUWING: Het lijkt erop dat het geen geldig item is om te kopen. Neem contact op met een beheerder!"
                },
                {
                    "MessageErrorItemNoValidsell",
                    "WAARSCHUWING: Het lijkt erop dat het geen geldig item is om te verkopen. Neem contact op met een beheerder!"
                },
                {
                    "MessageErrorRedeemKit",
                    "WAARSCHUWING: Er is een fout opgetreden bij het overhandigen van deze kit. Neem contact op met een beheerder!"
                },
                ...
                {
                    "MessageErrorItemNoExist",
                    "WAARSCHUWING: Het artikel dat u probeert te kopen, lijkt niet te bestaan! Neem contact op met een beheerder!"
                },
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop no va rebre cap resposta del connector Economics. Assegureu-vos que el connector d'Economia està instal·lat correctament."
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop no ha rebut cap resposta del connector ServerRewards. Assegureu-vos que el connector ServerRewards està instal·lat correctament."
                },
                ...
                {
                    "MessageErrorItemItem",
                    "ADVERTÈNCIA: Sembla que aquest article de venda que teniu no és un article vàlid. Poseu-vos en contacte amb un administrador."
                },
                {
                    "MessageErrorItemNoValidbuy",
                    "ADVERTÈNCIA: Sembla que no és un article vàlid per comprar, poseu-vos en contacte amb un administrador."
                },
                {
                    "MessageErrorItemNoValidsell",
                    "ADVERTÈNCIA: Sembla que no és un article vàlid per vendre, poseu-vos en contacte amb un administrador."
                },
                {
                    "MessageErrorRedeemKit",
                    "ADVERTÈNCIA: S&#39;ha produït un error en donar-vos aquest kit. Poseu-vos en contacte amb un administrador."
                },
                ...
                {
                    "MessageErrorItemNoExist",
                    "ADVERTÈNCIA: sembla que no existeix l&#39;article que intenteu comprar. Poseu-vos en contacte amb un administrador."
                },
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "O GUIShop não recebeu resposta do plugin Economics. Certifique-se de que o plugin Economics está instalado corretamente. "
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop não recebeu uma resposta do plugin ServerRewards. Certifique-se de que o plugin ServerRewards esteja instalado corretamente. "
                },
                ...
                {
                    "MessageErrorItemItem",
                    "AVISO: Parece que este item de venda que você tem não é um item válido! Entre em contato com um administrador!"
                },
                ...
                {
                    "MessageErrorItemNoExist",
                    "AVISO: o item que você está tentando comprar parece não existir! Entre em contato com um administrador!"
                },
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                {
                    "MessageShowNoEconomics",
                    "GUIShop hat keine Antwort vom Economics-Plugin erhalten. Bitte stellen Sie sicher, dass das Economics-Plugin korrekt installiert ist."
                },
                {
                    "MessageShowNoServerRewards",
                    "GUIShop hat keine Antwort vom ServerRewards-Plugin erhalten. Bitte stellen Sie sicher, dass das ServerRewards-Plugin korrekt installiert ist."
                },
                ...
                {
                    "MessageErrorItemItem",
                    "WARNUNG: Es scheint, dass dieser Verkaufsartikel, den Sie haben, kein gültiger Artikel ist! Bitte wenden Sie sich an einen Administrator!"
                },
                {
                    "MessageErrorItemNoValidbuy",
                    "WARNUNG: Es scheint, als wäre es kein gültiger Artikel zum Kaufen. Bitte wenden Sie sich an einen Administrator!"
                },
                {
                    "MessageErrorItemNoValidsell",
                    "WARNUNG: Es scheint, dass es sich nicht um einen gültigen Artikel handelt. Bitte wenden Sie sich an einen Administrator."
                },
                {
                    "MessageErrorRedeemKit",
                    "WARNUNG: Bei der Bereitstellung dieses Kits ist ein Fehler aufgetreten. Bitte wenden Sie sich an einen Administrator."
                },
                ...
                {
                    "MessageErrorSellPrice",
                    "WARNUNG: Der Administrator hat keinen Verkaufspreis angegeben. Sie können diesen Artikel nicht verkaufen"
                },
                ...
                {
                    "MessageErrorItemNoExist",
                    "WARNUNG: Der Artikel, den Sie kaufen möchten, scheint nicht zu existieren! Bitte wenden Sie sich an einen Administrator!"
                },
                {"MessageErrorItemNoExistTake", "Der Artikel, den Sie verkaufen möchten, ist derzeit nicht verfügbar."},
                ...
                {"Incendiary Pistol Bullet", "Pistolen Brandmunition"},
                ...
                {"12 Gauge Buckshot", "12 Gauge Buckshot"},
                {"12 Gauge Incendiary Shell", "12 Gauge Brandpatrone"},
                {"12 Gauge Slug", "12 Gauge Slug"},
                ...
                {"Netting", "Netz"},
                {"Shop Front", "Shop Front"},
                {"Metal Shop Front", "Metall Shop Front"},
                {"Metal Window Bars", "Metallfensterstangen"},
                {"Reinforced Window Bars", "Verstärkte Fensterstangen"},
                {"Wooden Window Bars", "Fensterläden aus Holz"},
                {"Metal horizontal embrasure", "Horizontale Metallverkleidung"},
                {"Metal Vertical embrasure", "Vertikale Metallverkleidung"},
                ...
                {"Rifle Body", "Gewehrgehäuse"},
                {"Semi Automatic Body", "Halbautomatisches Gehäuse"},
                {"SMG Body", "SMG Gehäuse"},
                {"Concrete Barricade", "Betonbarrikade"},
                {"Wooden Barricade Cover", "Holz Barrikaden Deckung"},
                ...
                {"Blue Boomer", "Blaues Feuerwerk"},
                {"Champagne Boomer", "Champagner Feuerwerk"},
                {"Green Boomer", "Grünes Feuerwerk"},
                {"Orange Boomer", "Oranges Feuerwerk"},
                {"Red Boomer", "Rotes Feuerwerk"},
                {"Violet Boomer", "Violettes Feuerwerk"},
                ...
                {"Wooden Floor Spikes", "Holzbodenspitze"},
                {"Fridge", "Kühlschrank"},
                {"Large Furnace", "Großer Ofen"},
                {"Furnace", "Ofen"},
                {"Hitch & Trough", "Futtertrog"},
                ...
                {"Fluid Combiner", "Wasser Kombinierer"},
        ...
        class Request
        {
            ...
            void RequestCallback(int code, string message)
            {
        ...
        /*private void OnNewSave(string filename) // TODO adding in wipe cooldown
        {
             _config.Time.WasWiped = true;
        ...
        private void OnEntityTakeDamage(BasePlayer player, HitInfo info) //Auto close feature
        {
            ...
            if (_playerUIOpen.Contains(player.UserIDString) && (info.IsProjectile() || info.damageTypes.Has(DamageType.Bite) ||
        ...
        private void Loaded()
        {
            ...
            {
                if (category.Permission.IsNullOrEmpty() || permission.PermissionExists(category.Permission, this)) continue;

                permission.RegisterPermission(category.Permission, this);
            }
            
            foreach (var player in BasePlayer.activePlayerList) CreateGUI(player);
        }

        private void Unload()
        {
            foreach (BasePlayer player in BasePlayer.activePlayerList) DestroyUi(player, true);
            foreach (BasePlayer player in BasePlayer.activePlayerList) DestroyGUI(player);
            SaveData();
            _instance = null;
        }
        ...
        private void OnUserPermissionGranted(string id, string permName)
        {
            ...
            {
                Puts($"This user {id} is already in a group that has guishop.use please remove duplication");
            }
            if (permName.Equals(Button))
                CreateGUI(BasePlayer.Find(id));
        }
        ...
        private CuiElementContainer CreateShopOverlay(string shopName, BasePlayer player, bool toggle)
        {
            ...
            container.Add(new CuiElement //Background image fix
            {
                ...
                Components =
                {
                    ...
                    {
                        Png = ImageLibrary?.Call<string>("GetImage", BackgroundImage) //updated 2.0.7
                    },
            ...
            if (toggle)
            {
                container.Add(new CuiElement // GUIShop Welcome MSG
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", WelcomeImage)
                        },
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.3 0.85",
                ...
                container.Add(new CuiElement // Amount Icon
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", Amount1Image) //2.0.7
                        },
                        new CuiRectTransformComponent
                ...
                container.Add(new CuiElement // Buy Icon
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", BuyImage) //2.0.7
                        },
                        new CuiRectTransformComponent
                ...
                container.Add(new CuiElement // Sell Icon
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", SellImage) //2.0.7
                        },
                        new CuiRectTransformComponent
                        {
                ...
                container.Add(new CuiElement // Amount Icon
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", Amount2Image) //2.0.7
                        },
                        new CuiRectTransformComponent
                ...
                container.Add(new CuiElement //close button image
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", CloseImage) //2.0.7
                        },
                        new CuiRectTransformComponent
            ...
            container.Add(new CuiLabel
                ...
                ShopOverlayName);
/*
            container.Add(new CuiLabel  //Adding missing Lable for limit function
                {
                    Text = {
                        Text = "Limit",
                        FontSize = 20,
        ...
        private CuiElementContainer CreateShopItemEntry(ShopItem shopItem, float ymax, float ymin, string shopName, string color,
            bool pricebuttons,
            bool cooldown, BasePlayer player) //bool HasBuyLimit, bool HasSellLimit) //add _limitsData, Semi finished
        {
            CuiElementContainer container = new CuiElementContainer();
/*
            if (HasBuyLimit && pricebuttons)
            {
                container.Add(new CuiLabel //Test added for Limits display set amount positioning (Its in the perfect position now!)
                    {
                        Text = {
                            Text = $"{shopItem.BuyLimit}",
                            FontSize = 15,
                            Color = GetUITextColor(player, _config.TextColor),
                            Align = TextAnchor.MiddleCenter
                        },
                        RectTransform = {
                            AnchorMin = $"{(pricebuttons ? 0.725 : 0.2)} {ymin}", //Keep location setting
                            AnchorMax = $"{(pricebuttons ? 0.5 : 0.5)} {ymax}" //Keep location setting
                        }
                    },
                    ShopContentName);
            }
            
            if (!pricebuttons && HasSellLimit)
            {
                container.Add(new CuiLabel //Test added for Limits display set amount positioning (Its in the perfect position now!)
                    {
                        Text = {
                            Text = $"{shopItem.SellLimit}",
                            FontSize = 15,
                            Color = GetUITextColor(player, _config.TextColor),
                            Align = TextAnchor.MiddleCenter
                        },
            ...
            container.Add(new CuiLabel //Buy + sell Price Display's Cost set amount in config
                {
                    Text =
                    {
                        Text = $"{(pricebuttons ? shopItem.SellPrice : shopItem.BuyPrice)}",
                        FontSize = 15,
                        Color = GetUITextColor(player, _config.TextColor),
                        Align = TextAnchor.MiddleLeft
                    },
                    RectTransform =
                    {
                        AnchorMin = $"{(pricebuttons ? 0.725 : 0.45)} {ymin}",
            ...
            int[] maxSteps = pricebuttons ? shopItem.SellQuantity.Length == 0 ? _config.steps : shopItem.SellQuantity : shopItem.BuyQuantity.Length == 0 ? _config.steps : shopItem.BuyQuantity;
            ...
            for (var i = 0; i < maxSteps.Length; i++)
            {
                container.Add(new CuiButton
                {
                    ...
                    {
                        Command =
                            $"shop.{(pricebuttons ? "Sell" : "buy")} {shopName.Replace(" ", "_")} {shopItem.DisplayName.Replace(" ", "_")} {maxSteps[i]}",
                        Color = color
                    },
                    RectTransform =
                    {
            ...
            if (_config.AllButton && !isKitOrCommand && !(!pricebuttons && shopItem.BuyCooldown > 0 || pricebuttons && shopItem.SellCooldown > 0))
            {
                container.Add(new CuiButton
                {
                    Button =
                    {
                        Command =
                            $"shop.{(pricebuttons ? "Sell" : "buy")} {shopName.Replace(" ", "_")} {shopItem.DisplayName.Replace(" ", "_")} all",
                        Color = color
                    },
                    RectTransform =
                    {
                        AnchorMin = $"{(pricebuttons ? 0.775 : 0.5) + maxSteps.Length * 0.03 + 0.001} {ymin}",
                        AnchorMax = $"{(pricebuttons ? 0.805 : 0.53) + maxSteps.Length * 0.03 - 0.001} {ymax}"
                    },
                    Text =
                    {
                        Text = "All", //All button
        ...
        private void CreateShopItemIcon(ref CuiElementContainer container, string name, float ymax, float ymin, ShopItem data,
            BasePlayer player)
        {
            ...
            //Puts($"{data.Image}, {data.Shortname}, {(bool) (ImageLibrary?.Call("HasImage", data.Shortname, data.SkinId) ?? false)}");
            if (!data.Image.IsNullOrEmpty())
            {
                rawImage.Png = (string) ImageLibrary?.Call("GetImage", data.Image, 0UL, false);
            }
            else if ((bool) (ImageLibrary?.Call("HasImage", data.Shortname, data.SkinId) ?? false))
            {
                rawImage.Png = (string) ImageLibrary?.Call("GetImage", data.Shortname, data.SkinId, false);
                //Puts($"{data.Image}");
            }
            else
            {
            ...
            container.Add(label, ShopContentName);
        ...
        private void CreateShopColorChanger(ref CuiElementContainer container, string currentshop, BasePlayer player, bool toggle)
        {
            container.Add(new CuiLabel
                {
                    Text =
            ...
            container.Add(new CuiLabel //Display Bar
                {
                    ...
                    RectTransform = {AnchorMin = "0.80 0.19", AnchorMax = $"{0.80 + AnchorBarMath(player)} 0.24"}
                },
                ShopOverlayName);
            ...
            if (toggle)
            {
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", ForwardArrowImage)
                        },
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.85 0.14",
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent
                        {
                            Png = ImageLibrary?.Call<string>("GetImage", BackArrowImage)
                        },
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.80 0.14",
        ...
        private void CreateShopChangePage(ref CuiElementContainer container, string currentshop, int shoppageminus, int shoppageplus,
            BasePlayer player, bool toggle)
        {
            if (toggle)
            {
                container.Add(new CuiElement
        ...
        private void CreateTab(ref CuiElementContainer container, ShopCategory cat, int shoppageminus, int rowPos,
            BasePlayer player) //Button-Shop Tab generator
        {
            container.Add(new CuiButton
            {
                Button =
                ...
                Text =
                {
                    Text = Lang(cat.DisplayName, player.UserIDString),
                    Align = TextAnchor.MiddleCenter,
                    Color = GetUITextColor(player, cat.DisplayNameColor ?? _config.TextColor),
                    Font = "robotocondensed-regular.ttf",
                    FontSize = 12
                }
        ...
        private void CreateGUI(BasePlayer player)
        {
            ...
            elements.Add(new CuiElement
            {
                Parent = Button,
                Components = {
                    new CuiRawImageComponent { Png = ImageLibrary?.Call<string>("GetImage", ShopButton) },
            ...
            {
                Button = { Command = "shop.button", Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                Text = { Text = "" }
            }, shopButton);
        ...
        public int GetAmount(PlayerInventory inventory, int itemid, ulong skinId = 0UL)
        {
            if (itemid == 0)
                return 0;
            int num = 0;
            ...
            if (inventory.containerWear != null)
                num += GetAmount(inventory.containerWear, itemid, skinId, true);
            return num;
        }

        public int GetAmount(ItemContainer container, int itemid, ulong skinId, bool onlyUsableAmounts)
        {
            int num = 0;
            foreach (Item obj in container.itemList)
            {
                if (obj.info.itemid == itemid && obj.skin == skinId && (!onlyUsableAmounts || !obj.IsBusy()))
                    num += obj.amount;
            }
        ...
        private double GetCurrency(ShopCategory shop, BasePlayer player)
        {
            ...
            {
                return GetAmount(player.inventory, shop.CustomCurrencyIDs, shop.CustomCurrencySkinIDs);
            }
            ...
            {
                return GetAmount(player.inventory, _config.CustomCurrencyID, _config.CustomCurrencySkinID);
            }

            return 0;
        }

        public int Take(PlayerInventory inventory, List<Item> collect, int itemid, int amount, ulong skinId = 0UL)
        {
            int num1 = 0;
            if (inventory.containerMain != null)
            {
                int num2 = Take(inventory.containerMain, collect, itemid, amount, skinId);
                num1 += num2;
                amount -= num2;
            }
            ...
            if (inventory.containerBelt != null)
            {
                int num2 = Take(inventory.containerBelt, collect, itemid, amount, skinId);
                num1 += num2;
                amount -= num2;
            }

            if (amount <= 0 || inventory.containerWear == null)
                return num1;
            int num3 = Take(inventory.containerWear, collect, itemid, amount, skinId);
            num1 += num3;
            amount -= num3;
            return num1;
        }

        public int Take(ItemContainer container, List<Item> collect, int itemid, int iAmount, ulong skinId)
        {
            int num1 = 0;
            if (iAmount == 0)
                return num1;
        ...
        public int GetAmountPrice(double amount)
        {
            if (amount <= 0.5)
            {
        ...
        {
            //Puts($"{amount} {GetAmount(player.inventory, shop.CustomCurrencyIDs, shop.CustomCurrencySkinIDs)}");
            if (shop?.Currency.Contains("custom") == true &&
                GetAmount(player.inventory, shop.CustomCurrencyIDs, shop.CustomCurrencySkinIDs) >= GetAmountPrice(amount))
            {
                Take(player.inventory, null, shop.CustomCurrencyIDs, GetAmountPrice(amount), shop.CustomCurrencySkinIDs);
                return true;
            }

            if (_config.Economics && Economics != null || Economics != null && shop?.Currency.Contains("economics") == true)
            {
                //Puts($"withdrawing {(double)amount} <double format int> {(int)amount} from {player.displayName}");
            ...
            //Puts($"{GetAmount(player.inventory, _config.CustomCurrencyID, _config.CustomCurrencySkinID) >= amount}, {GetAmountPrice(amount)}, {amount}"); 
            if (_config.CustomCurrency && GetAmount(player.inventory, _config.CustomCurrencyID, _config.CustomCurrencySkinID) >= GetAmountPrice(amount))
            {
                Take(player.inventory, null, _config.CustomCurrencyID, GetAmountPrice(amount), _config.CustomCurrencySkinID);
        ...
        private bool NearNpc(string npcId, Vector3 position)
        {
            ...
            return Vector3Ex.Distance2D(npc.ServerPosition, position) <= _config.NPCDistanceCheck;
        }
        ...
        private void ShowShop(BasePlayer player, string shopid, int from = 0, bool fullPaint = true, bool refreshMoney = false)
        {
            ...
            shopDescription.Text.Color = GetUITextColor(player, shop.DescriptionColor ?? _config.TextColor);
            ...
            if (fullPaint)
            {
                container = CreateShopOverlay(shop.DisplayName, player, toggle);
                ...
                {
                    CreateShopColorChanger(ref container, shopid, player, toggle); //2.0.7 updating UI
                }
                ...
                {
                    foreach (ShopCategory cat in _config.ShopCategories.Values.Where(i => i.EnableNPC && i.NPCId == shop.NPCId))
                    {
                        if (!string.IsNullOrEmpty(cat.Permission) &&
                            !permission.UserHasPermission(player.UserIDString, cat.Permission)) continue;
            ...
            /*if (_config.sellbuttons)
            {
                container.Add(new CuiPanel
                {
                    ...
                    RectTransform =
                    {
                        AnchorMin = "0.675 0.2", //Second digit = Hieght Done. First Digit = Position on screen from left to right.
                        AnchorMax = "0.955 0.6" //Left to right size for msg
                    }
                }, ShopOverlayName, ShopContentName2);
            }*/
            ...
            foreach (ShopItem data in shopItems)
            {
                if (current >= from && current < from + 7)
                {
                    ...
                    name =
                        $"{Lang(name, player.UserIDString)}<size=10>{(cooldown ? "\n" + cooldowndescription : "")}</size>"; //added Updated,  Creates new line for cooldowns under the Displayed Item Names.

                    CreateShopItemIcon(ref container, name, pos + 0.125f, pos, data, player);
                    ...
                    if (!hasBuyed && data.EnableBuy)
                        container.AddRange(CreateShopItemEntry(data, pos + 0.125f, pos, shopid, GetUIBuyBoxColor(player), false, hasBuyCooldown,
                            player)); //hasBuyCooldown, player, itemLimit.HasBuyLimit(data.Shortname, data.BuyLimit), itemLimit.HasSellLimit(data.Shortname, data.SellLimit)));

                    if (data.EnableSell)
                        container.AddRange(CreateShopItemEntry(data, pos + 0.125f, pos, shopid, GetUISellBoxColor(player), true,
                            hasSellCooldown,
                            player)); //hasSellCooldown, player, itemLimit.HasBuyLimit(data.Shortname, data.BuyLimit), itemLimit.HasSellLimit(data.Shortname, data.SellLimit)));
                }
            ...
            CreateShopChangePage(ref container, shopid, minfrom, maxfrom, player, toggle);

            CuiHelper.AddUi(player, container);
        }

        object CanDoAction(BasePlayer player, string shopName, string item, string ttype)
        {
        ...
        object TryShopBuy(BasePlayer player, string shopName, string item, int amount)
        {
            var shop = _config.ShopCategories[shopName];
            if (amount <= 0)
            {
                return false;
            }

            object success = CanShop(player, shopName);
            ...
            success = CanDoAction(player, shopName, item, "buy");
            ...
            success = CanBuy(player, shop, item, amount);
            ...
            success = TryGive(player, shop, item, amount);
            ...
            if (!TakeCurrency(shop, player, (data.BuyPrice * amount)))
            {
                return Lang("MessageErrorTakeCurrency", player.UserIDString, data.Shortname);
            }
        ...
        object TryGive(BasePlayer player, ShopCategory shopCategory, string item, int amount)
        {
            ShopItem data = _config.ShopItems[item];
            ...
            if (data.RunCommandAndCustomShopItem && !data.Command.IsNullOrEmpty())
            {
                ...
                embed.AddField("Cost", $"{amount * data.BuyPrice}", true)
                    .AddField("Remaining Balance", $"{GetCurrency(shopCategory, player) - amount * data.BuyPrice}", true);
                embeds.Add(embed);
            ...
            else if (!data.Command.IsNullOrEmpty()) //updated/patched sorta 5/13/2021
            {
                Vector3 pos = GetLookPoint(player);
            ...
            else if (!string.IsNullOrEmpty(data.KitName))
            {
                ...
                if (!(isKit is bool) || !(bool) isKit)
                {
                    ...
                    SendToDiscord(new List<Embed> {embed});
                    return Lang("MessageErrorNotKit", player.UserIDString, data.DisplayName);
                }
        ...
        private int GetAmountBuy(BasePlayer player, ShopCategory shopCategory, string item)
        {
            ...
            var amount = (int) Math.Floor(GetCurrency(shopCategory, player) / data.BuyPrice);

            if (amount >= freeSlots * definition.stackable)
            {
        ...
        private object GiveItem(BasePlayer player, ShopItem data, int amount)
        {
            ...
            {
                var item = ItemManager.CreateByName(data.MakeBlueprint ? "blueprintbase" : data.Shortname, quantity, data.SkinId); //ItemManager.CreateByItemID(definition.itemid, quantity, data.SkinId);
                ...
                {
                    if (ItemManager.FindBlueprint(definition) == null) return Lang("MessageErrorItemIsNotBlueprintable", player.UserIDString, data.DisplayName);
        ...
        object CanBuy(BasePlayer player, ShopCategory shop, string item, int amount)
        {
            ...
            if (GetCurrency(shop, player) < buyprice * amount)
            {
                if (shop?.Currency.Contains("custom") == true)
                {
                    Item currency = ItemManager.CreateByItemID(shop.CustomCurrencyIDs, amount, shop.CustomCurrencySkinIDs);
                    if (currency == null)
                    {
                        return Lang("MessageErrorCustomCurrency", player.UserIDString, shop.DisplayName, shop.CustomCurrencyIDs);
                    }
                }
                if (_config.CustomCurrency)
                {
            ...
            if (data.BuyCooldown > 0)
            {
                ...
                if (_buyCooldownData.TryGetValue(player.userID, out itemCooldowns) && itemCooldowns.TryGetValue(item, out itemCooldown) &&
        ...
        object TryShopSell(BasePlayer player, string shopName, string item, int amount)
        {
            var shop = _config.ShopCategories[shopName];
            object success = CanShop(player, shopName);
            if (success is string)
            ...
            success = CanDoAction(player, shopName, item, "Sell");
            if (success is string)
            {
                return success;
            }
            ...
            success = TrySell(player, shopName, item, amount);
            if (success is string)
            {
                return success;
            }
            ...
            AddCurrency(shop, player, (data.SellPrice * amount), item);
        ...
        object TrySell(BasePlayer player, string shopName, string item, int amount)
        {
            var shop = _config.ShopCategories[shopName];
            Embed embed = BuildMessage(player);
            ShopItem data = _config.ShopItems[item];
            ...
            object success = TakeItem(player, data, shop, amount);
            if (success is string)
            {
                return success;
            }
            ...
            if (_config.EnableDiscordSellLogging)
            {
                embed.AddTitle("Item Sold").AddField("Item Name", data.Shortname).AddField("amount", $"{amount}", true)
                    .AddField("Receive", $"{amount * data.SellPrice}", true)
                    .AddField("Balance", $"{GetCurrency(shop, player) + amount * data.SellPrice}", true);
                SendToDiscord(new List<Embed>{embed});
            }
        ...
        private int GetAmountSell(BasePlayer player, string item)
        {
            ...
            return GetAmount(player.inventory, definition.itemid);
        }

        private object TakeItem(BasePlayer player, ShopItem data, ShopCategory shop, int amount)
        {
            if (amount <= 0)
            {
                Puts("amount is <= 0 true");
            ...
            int pamount = GetAmount(player.inventory, definition.itemid, data.SkinId);

            if (pamount < amount)
            {
                Puts("TakeItem pamount < amount check true ");
                return Lang("MessageErrorNotEnoughSell", player.UserIDString);
            }
            ...
            Take(player.inventory, null, definition.itemid, amount, data.SkinId);
        ...
        object CanSell(BasePlayer player, string item, int amount)
        {
            ...
            if (itemdata.Command.IsNullOrEmpty() && player.inventory.containerMain.FindItemsByItemName(itemdata.Shortname) == null &&
                player.inventory.containerBelt.FindItemsByItemName(itemdata.Shortname) == null) //fixed..
            {
                //Puts($"Is inside CanSell dunno why.. {itemdata.Shortname}");
                return Lang("MessageErrorNotEnoughSell", player.UserIDString);
            }
            ...
            if (itemdata.SellCooldown > 0)
            {
                ...
                if (_sellCoolDownData.TryGetValue(player.userID, out itemCooldowns) && itemCooldowns.TryGetValue(item, out itemCooldown) &&
                    itemCooldown > CurrentTime())
                {
                    return Lang("MessageErrorCooldown", player.UserIDString, FormatTime((long) (itemCooldown - CurrentTime())));
                }
        ...
        private void CmdShop(BasePlayer player, string command, string[] args)
        {
            ...
            else if (String.IsNullOrEmpty(_config.DefaultShop))
            {
                SendReply(player, Lang("MessageErrorGlobalDisabled", player.UserIDString));
                return;
            }
        ...
        private void ToggleBackpackGUI(BasePlayer player, string cmd, string[] args)
        {
            ...
            SendReply(player, "Toggled GUIShop Button {0}", _playerDisabledButtonData.Contains(player.UserIDString)?"Disabled" : "Enabled", this, player.UserIDString);
            SaveData();
        }
        ...
        [ConsoleCommand("clear.limit")]
        private void ConsoleClearLimit(ConsoleSystem.Arg arg)
        {
        ...
        private void ConsoleShopButton(ConsoleSystem.Arg arg)
        {
            CmdShop(arg.Player(), null, null);
        }
        ...
        private void ConsoleShopBuy(ConsoleSystem.Arg arg, ShopCategory shopCategory, string items)
        {
            if (!arg.HasArgs(3))
            {
                return;
        ...
        private void ConsoleYes(ConsoleSystem.Arg arg)
        {
        ...
        private bool HasBuyCooldown(ulong userID, string item, out double itemCooldown)
        {
            ...
            return _buyCooldownData.TryGetValue(userID, out itemCooldowns) && itemCooldowns.TryGetValue(item, out itemCooldown) &&
                   itemCooldown > CurrentTime();
        }
        ...
        private bool HasSellCooldown(ulong userID, string item, out double itemCooldown)
        {
            ...
            return _sellCoolDownData.TryGetValue(userID, out itemCooldowns) && itemCooldowns.TryGetValue(item, out itemCooldown) &&
                   itemCooldown > CurrentTime();
        }
        ...
        private double AnchorBarMath(BasePlayer uiPlayer) => (GetUITransparency(uiPlayer) / 10 -
                                                              (GetUITransparency(uiPlayer) / 10 - GetPlayerData(uiPlayer).RangeValue / 1000)) *
                                                             10;
        ...
        private void PlayerColorTextChange(BasePlayer uiPlayer, string textColorRed, string textColorGreen, string textColorBlue,
            string uiSettingToChange)
        {
            PlayerUISetting playerUISetting = GetPlayerData(uiPlayer);
        ...
        private bool LimitReached(ulong userId, string item, int amount, string ttype) //Limiter function TODO:
        {
            ...
            if (ttype == "BuyPrice" && !itemLimit.HasBuyLimit(item, amount))
            {
            ...
            if (ttype == "SellPrice" && !itemLimit.HasSellLimit(item, amount))
            {
        ...
        bool GetNearestVendor(BasePlayer player, out ShopCategory category) //NPC helper finished.
        {
            ...
            category = _config.ShopCategories.Select(x => x.Value)
                .FirstOrDefault(i => i.EnableNPC && i.NPCId == npc.UserIDString); //updated 1.8.1
        ...
        private void OnUseNPC(BasePlayer npc, BasePlayer player) //added 1.8.7
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, Use))
            {
                SendReply(player, "You do not have permission guishop.use");
                return;
            }

            float distance = Vector3.Distance(npc.ServerPosition, player.ServerPosition);
            if (distance > _config.NPCDistanceCheck) return;
            ShopCategory category = _config.ShopCategories.Select(x => x.Value).FirstOrDefault(i =>
                i.EnableNPC && i.NPCId == npc.UserIDString &&
                (string.IsNullOrEmpty(i.Permission) || permission.UserHasPermission(player.UserIDString, i.Permission)));
            if (category == null)
            {
                return;
            }
        ...
        private Vector3 GetLookPoint(BasePlayer player)
        {
            ...
            if (!Physics.Raycast(new Ray(player.eyes.position, Quaternion.Euler(player.serverInput.current.aimAngles) * Vector3.forward),
                out raycastHit, _config.SpawnDistance))
            {
        ...
        private double GetCurrency(BasePlayer player, string shopName)
        {
            ShopCategory shop;
            if (!_config.ShopCategories.TryGetValue(shopName, out shop))
            {
                return 0;
            }
        ...
        {
            ShopCategory shop;
            if (!_config.ShopCategories.TryGetValue(shopName, out shop) || !shop.EnableNPC || shop.NPCId != npcID)
            {
                return;
            }
